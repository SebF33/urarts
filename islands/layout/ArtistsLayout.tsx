...
export default function ArtistsLayout(
  ...
  const displayedArtists = display
    ? props.artists.slice(0, displayedArtistIndex + NB_LOADING_ARTISTS)
    : [];
  ...
  useEffect(() => {
    ...
    const timeoutNoresults = setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const noresultsElement: HTMLElement | null = document.querySelector(
      "#Noresults",
    );
      ...
      if (artistElement) {
        tippy(artistElement, {
          ...
          content:
            ...
            <p style="font-size:1em;line-height:1.05;padding-bottom:8px">${
              i18next.t("artists.nationality", { ns: "translation" })
            } ${p.nationality}</p>
          ...
          <div class={`${props.grid}`}>
            ...
            {displayedArtists.map((p, index) => (
              <div
                key={index + 1}
                data-artist-id={p.id}
                class="artist-frame transform-gpu appear-effect-very-fast-fadein bg-dark relative pb-[120%] shadow-[0_10px_7px_-5px_rgba(0,0,0,0.3)]"
              >
                <p class="text-lighterdark leading-3 pr-2 z-10 absolute top-[6.5%] bottom-[3%] left-[10.5%] right-[2.5%] text-[calc(0.8em+0.16vw)]">
                  {p.first_name} {p.last_name}
                </p>
                {p.signature && (
                  <img
                    class="w-12 z-10 absolute top-[86.0303%] left-[70.5%] text-[calc(0.6em+0.5vw)]"
                    src={p.signature}
                    alt={p.signature}
                    draggable={draggable}
                  />
                ...
                {display && (
                  <div class="absolute inset-[3.0303%_2.5%] bg-realwhite shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]">
                    <a
                      href={"/art/" + p.slug}
                      onClick={delayedClientNavigation}
                      draggable={draggable}
                      class={`
                        group flex justify-center text-center overflow-hidden z-20 cursor-pointer
                        absolute inset-[16.129%_13.158%] bg-[#ddc]
                        after:content-[''] after:block after:absolute after:top-0 after:w-full after:h-full
                        ${
                          imageOnLoadStyle.fullSizeNoTransition
                            ? "after:shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]"
                            : ""
                        }
                      `}
                    >
                      ...
                      />
                      <div
                        class={`absolute w-full h-full bg-black opacity-0 transition-opacity duration-[400ms] group-hover:opacity-60`}
                      />
                    </a>
                  </div>
                )}
          ...
          <div class={`${props.grid}`}>
            {showPlaceholder
              ? (
                <div
                  id="Noresults"
                  class="transform-gpu appear-effect-fast-fadein bg-dark relative pb-[120%] shadow-[0_10px_7px_-5px_rgba(0,0,0,0.3)]"
                >
                  <div class="absolute inset-[3.0303%_2.5%] bg-realwhite shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]">
                    <div
                      class="
                        group flex justify-center text-center overflow-hidden z-20
                        absolute inset-[16.129%_13.158%] bg-[#ddc]
                        after:content-[''] after:block after:absolute after:top-0 after:w-full after:h-full
                        after:shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]
                      "
